...
export const DecisionCard = ({ signal, userRole }: DecisionCardProps) => {
    ...
    // Generate match intelligence data (simulated - in production fetch from API)
    const matchIntelligence = {
        winProbability: {
            teamA: signal.event.isHot ? 55 : 48,
            teamB: signal.event.isHot ? 45 : 52,
        },
        h2hRecord: `Last 5 meetings: ${signal.event.teamA} 3W-1D-1L ${signal.event.teamB}`,
        keyHighlight: signal.event.isHot
            ? `🔥 Title decider! Both teams fighting for championship.`
            : `Regular season clash - Strong defensive record for both sides.`,
        newsLinks: [
            {
                title: `${signal.event.teamA} vs ${signal.event.teamB} Preview`,
                url: `https://www.google.com/search?q=${encodeURIComponent(signal.event.title + ' preview')}`
            },
            {
                title: 'Live Odds & Betting Stats',
                url: `https://www.google.com/search?q=${encodeURIComponent(signal.event.title + ' odds')}`
            }
        ]
    };
    ...
    return (
        <div className="bg-mosport-card rounded-xl border border-gray-800 overflow-hidden mb-6 transition-all hover:border-gray-700">
            {/* === MATCH HEADER (Clickable to expand match intel) === */}
            <div
                className="p-4 sm:p-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-gradient-to-r from-gray-900 to-mosport-card cursor-pointer hover:bg-gray-800/50 transition-all"
            ...
            >
                ...
                <div className="flex items-center gap-3">
                    ...
                    <ChevronDown
                        size={20}
                        className={`text-gray-400 transition-transform duration-300 ${isMatchExpanded ? 'rotate-180' : ''}`}
                    />
                </div>
            </div>

            {/* === EXPANDABLE MATCH INTELLIGENCE === */}
            <div className={`
                overflow-hidden transition-all duration-500 ease-in-out
                ${isMatchExpanded ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}
            `}>
                <div className="px-6 py-4 bg-black/30 border-t border-white/5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-3">
                                <span className="text-sm font-bold text-white w-32">{signal.event.teamA}</span>
                                <div className="flex-1 bg-gray-800 rounded-full h-2 overflow-hidden">
                                    <div
                                        className="bg-gradient-to-r from-blue-500 to-cyan-400 h-full transition-all"
                                        style={{ width: `${matchIntelligence.winProbability.teamA}%` }}
                                    />
                                </div>
                                <span className="text-sm font-bold text-cyan-400 w-12 text-right">
                                    {matchIntelligence.winProbability.teamA}%
                                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="text-sm font-bold text-white w-32">{signal.event.teamB}</span>
                                <div className="flex-1 bg-gray-800 rounded-full h-2 overflow-hidden">
                                    <div
                                        className="bg-gradient-to-r from-purple-500 to-pink-400 h-full transition-all"
                                        style={{ width: `${matchIntelligence.winProbability.teamB}%` }}
                                    />
                                </div>
                                <span className="text-sm font-bold text-pink-400 w-12 text-right">
                                    {matchIntelligence.winProbability.teamB}%
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="mb-4">
                        ...
                        <p className="text-sm text-gray-200 leading-relaxed">
                            {matchIntelligence.keyHighlight}
                        </p>
                    </div>

                    {/* News Links */}
                    <div className="flex items-center gap-4 flex-wrap">
                        {matchIntelligence.newsLinks.map((link, idx) => (
                            <a
                                key={idx}
                                href={link.url}
                                target="_blank"
